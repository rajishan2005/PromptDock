...
4. **Autosolve** — sends every detected question to ChatGPT one after another automatically: types it in, submits it, waits for ChatGPT to finish answering, cools down 5 seconds, then moves to the next. Visible Start/Stop control; if a question fails to send or a reply can't be detected, it's skipped and the run continues rather than stopping.
